...
export function UploadPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div>
        <p className="text-sm font-bold uppercase text-brand">Upload</p>
        <h2 className="mt-1 text-3xl font-black">Add a handwritten note</h2>
      </div>
      ...
      <div
        ...
        className={`surface flex min-h-72 cursor-pointer flex-col items-center justify-center border-2 border-dashed p-6 text-center transition sm:min-h-96 sm:p-8 ${
          isDragActive ? 'border-brand bg-mist' : 'border-ink/15 hover:border-brand/50 hover:bg-white/80'
        }`}
      >
        <input {...getInputProps()} />
        ...
        <p className="mt-5 text-2xl font-black">{upload.isPending ? 'Uploading...' : 'Drop file to upload'}</p>
        <p className="mt-2 max-w-md text-ink/60">JPG, JPEG, PNG, or PDF up to 20 MB.</p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          <span className="badge bg-mist text-brand"><FileImage size={14} /> Images</span>
          <span className="badge bg-mist text-brand"><FileText size={14} /> PDFs</span>
        </div>
      </div>
